...
import { Plus, Pencil, Trash2, Search, ChefHat, X, Loader2, AlertCircle } from 'lucide-react';
import { recipeCategoriesApi } from '../api';

interface Category {
  id: number;
  name: string;
  description: string | null;
  _count?: { Recipe: number };
}
...
function RecipeCategories() {
  ...
  const loadCategories = async () => {
    try {
      const res = await recipeCategoriesApi.getAll();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editingCategory) {
        await recipeCategoriesApi.update(editingCategory.id, formData);
      } else {
        await recipeCategoriesApi.create(formData);
      }
      handleCloseModal();
      loadCategories();
    } catch (error: any) {
  ...
  const handleDelete = async (id: number) => {
    if (!window.confirm('Yakin ingin menghapus kategori resep ini?')) return;
    try {
      await recipeCategoriesApi.delete(id);
      loadCategories();
    } catch (error: any) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h1 className="text-2xl lg:text-3xl font-bold text-gray-900 dark:text-white">Kategori Resep</h1>
          <p className="text-gray-500 dark:text-dark-400 mt-1">Kelola kategori untuk mengelompokkan resep</p>
        </div>
      ...
      {filteredCategories.length === 0 ? (
        <div className="card">
          <div className="empty-state py-12">
            ...
            <h3 className="text-lg font-medium text-gray-900 dark:text-white mb-2">Tidak ada kategori resep</h3>
            <p className="text-sm">
              {searchTerm ? 'Tidak ditemukan kategori dengan kata kunci tersebut' : 'Mulai dengan menambahkan kategori resep'}
            </p>
          </div>
        </div>
      ) : (
                  ...
                  <tr key={category.id}>
                    ...
                    <td>
                      <span className="badge badge-primary">
                        {category._count?.Recipe || 0} resep
                      </span>
                    </td>
                    <td>
          ...
          <div className="modal max-w-md" onClick={(e) => e.stopPropagation()}>
            <div className="modal-header">
              <h2 className="text-lg font-semibold text-gray-900 dark:text-white">
                {editingCategory ? 'Edit Kategori Resep' : 'Tambah Kategori Resep'}
              </h2>
